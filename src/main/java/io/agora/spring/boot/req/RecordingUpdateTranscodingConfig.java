...
public class RecordingUpdateTranscodingConfig {
	...
	/**
     * 1、如果 layoutType 设为 2（垂直布局），用该参数指定显示大视窗画面的用户 ID。
     */
    ...
    /**
     * 2、视频画布的背景颜色。支持 RGB 颜色表，字符串格式为 # 号后 6 个十六进制数。默认值 "#000000" 黑色（选填）
     */
	@JsonProperty("backgroundColor")
    private String backgroundColor = "#000000";
    /**
     * 3、视频画布的背景图的 URL 地址。背景图的显示模式为裁剪模式。裁剪模式下，优先保证画面被填满。背景图尺寸等比缩放，直至整个画面被背景图填满。如果背景图长宽与显示窗口不同，则背景图会按照画面设置的比例进行周边裁剪后填满画面。（选填）
     */
	@JsonProperty("backgroundImage")
    private String backgroundImage;
    /**
     * 4、默认的用户画面背景图的 URL 地址。配置该参数后，当任一⽤户停止发送视频流超过 3.5 秒，画⾯将切换为该背景图；如果针对某 UID 单独设置了背景图，则该设置会被覆盖（选填）
     */
	@JsonProperty("defaultUserBackgroundImage")
    private String defaultUserBackgroundImage;
    /**
     * 5、由每个用户对应的布局画面设置组成的数组，支持最多 17 个用户画面。当 mixedVideoLayout 设为 3 时，可以通过该参数自定义合流布局。（选填）
     */
	@JsonProperty("layoutConfig")
    private List<TranscodingLayoutConfig> layoutConfig;
    /**
     * 6、数组内容为各 UID 单独的背景图设置。（选填）
     * notice: 
