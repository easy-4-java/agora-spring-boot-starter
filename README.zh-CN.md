...
> **当前版本**：`2.7.x-20240823.RELEASE`<br>
> **JDK 基线**：`17`<br>
...
**agora-spring-boot-starter** 是一个面向 使用 agora 的应用 的 Spring Boot Starter，用于将 **agora** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 agora 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 agora 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 agora Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:agora-spring-boot-starter:2.7.x-20240823.RELEASE` |
| 配置前缀 | `agora` |
...
|---|---:|---|
| JDK | `17` | `pom.xml` |
| Spring Boot | `2.7.0` | `pom.xml` parent |
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `agora.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/agora-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/agora-spring-boot-starter)
